...
class GameStore {
  ...
  /** Fraction …/1 : retire le dénominateur (x/1 → x). */
  clearOneDenominator(cardId: string) {
    if (!this.state) return;
    if (this.state.pending) {
      this.flashAlert();
      return;
    }
    this.applyState(clearOneDenominator(this.state, cardId));
  }
  ...
  tryDrop(sourceFractionId: string, target: { fractionId?: string; side?: "lhs" | "rhs"; holeCardId?: string; divideZone?: boolean; multiplyZone?: boolean }) {
    ...
    if (target.fractionId) {
      ...
      if (tgt && src.side === tgt.side) {
        ...
        if (
          this.caps.addFractionPower &&
          canAddFractions(this.state, sourceFractionId, target.fractionId)
        ) {
          const targetFractionId = target.fractionId;
